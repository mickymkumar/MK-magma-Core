...
```
✅ Can be safely ignored. It does not stop execution.

Undefined service dependency

```
service "waitForHealthyPods" depends on undefined service "gateway_ovs"
Since we are not modifying the files, this service will fail to start.

Workaround: start only the core AGW services:

```
docker compose up -d magmad pipelined
```
🔧 Step 7: Access Running Containers
```
docker ps                # list running containers
docker exec -it magmad bash   # enter magmad container
docker exec -it pipelined bash   # enter pipelined container
```
### Optional: Pull Prebuilt Docker Image
```
docker pull mickymkumar/magma-agw:latest
docker run -it --privileged --name magma-agw mickymkumar/magma-agw:latest bash
```
## References
Magma Documentation
Magma GitHub
